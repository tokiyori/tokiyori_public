    ...
    <article <?php post_class('post-list animated fadeIn'); ?> role="article">
      <a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
        <figure class="eyecatch<?php if (!has_post_thumbnail()): ?> noimg<?php endif; ?>">
          ...
          <?php
          // @ NO Images画像が表示されるようカスタマイズ
          // ------------------------------
          if ( has_post_thumbnail()){
              the_post_thumbnail('home-thum');
          }else{ ?>
            <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/img/noimages/noimages.jpg">
          <?php } ?>
        ...
        <section class="entry-content cf">
          ...
          <div class="byline entry-meta vcard">
            ...
            if ($post_options_date !== "date_off") : ?>
              <time class="date gf updated"><?php echo get_the_date(); ?></time><?php endif; ?>
            <?php if (get_option('post_options_authordisplay', 'author_off') == 'author_on'): ?><span
              class="writer name author"><?php echo get_avatar(get_the_author_meta('ID'), 30); ?><span
