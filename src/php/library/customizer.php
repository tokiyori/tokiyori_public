...
function opencage_logo_theme_customizer( $wp_customize ) {
	...
	$wp_customize->add_control( 'side_options_description', array(
	    'label' =>'キャッチフレーズを表示する',
		'description' => '<span style="font-size:10px;">サイトタイトル下に「キャッチフレーズ」を表示するかどうか。</span>',
	...
	$wp_customize->add_control( 'opencage_logo_size', array(
	    ...
	    'label' =>'ロゴサイズ変更',
		'description' => '<span style="font-size:10px;">ロゴのフォントサイズを変更できます。</span>',
	    'section' => 'title_tagline',
	    'type' => 'radio',
	    'choices' => array(
            'fs_s' => '小さめ',
	...
	$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'opencage_logo', array(
		...
		'description' => '<span style="font-size:10px;">ロゴ画像を利用する場合はこちらからアップロードしてください。</span>',
		'section'    => 'title_tagline',
...
#breadcrumb li a::after{color: <?php echo $maintext; ?>;}
a, #breadcrumb li a i, .authorbox .author_sns li a::before{color: <?php echo $mainlink; ?>;}
...
.widget li a:after{color: <?php echo $mainlink; ?>;}.accordionBtn{background-color: <?php echo $mainlink; ?>;}.accordionBtn.active{background-color: <?php echo $mainlinkhover; ?>;}
.entry-content h2,.widgettitle{background: <?php echo $mainttlbg; ?>; color: <?php echo $mainttltext; ?>;}
.entry-content h3,.entry-content h4{border-color: <?php echo $mainttlbg; ?>; color: <?php echo $maintext; ?>;}
...
.eyecatch .cat-name,ul.wpp-list li a:before{background: <?php echo $mainttlbg; ?>; color: <?php echo $mainttltext; ?>;}

.btn-wrap a{background: <?php echo $mainlink; ?>;border: 1px solid <?php echo $mainlink; ?>;}
.btn-wrap a:hover{color: <?php echo $mainlink; ?>;border-color: <?php echo $mainlink; ?>;}
...
.blue-btn, #submit, button, html input[type="button"], input[type="reset"], input[type="submit"] { background-color: <?php echo $mainlink; ?>; }
.blue-btn:hover, #submit:hover, .blue-btn:focus, #submit:focus {background-color: <?php echo $mainlinkhover; ?>; }

#footer-top::before{background: <?php echo $mainttlbg; ?>;}
#footer,.cta-inner{background-color: <?php echo $footerbg; ?>; color: <?php echo $footertext; ?>;}

.slick-prev:before, .slick-next:before{background-color: <?php echo $mainlink; ?>;}
...
function opencage_global_customizer($wp_customize) {
	...
	$wp_customize->add_control( 'side_options_gfnotojp', array(
		...
		'description' => '<span style="font-size:10px;">Googleフォント「NotoSansJapanese」を読み込まないようにすることができます。フォントデザインよりもサイト速度を気にされる方向け。</span>',
	    'section' => 'global_section',
	    'type' => 'radio',
	    'choices' => array(
            'notojp_on' => '読み込む',
	...
	$wp_customize->add_control( 'post_options_label', array(
	    ...
	    'label' =>'カテゴリーラベルの表示設定',
		'description' => '<span style="font-size:10px;">アイキャッチ画像の右上に表示されるカテゴリーラベルの表示非表示を設定できます。</span>',
	    'section' => 'global_section',
	    'type' => 'radio',
	    'choices' => array(
            'catlabelon' => 'ラベルを表示する',
	...
	$wp_customize->add_control( 'side_options_cataccordion', array(
		...
		'description' => '<span style="font-size:10px;">カテゴリーウィジェットのサブカテゴリーを隠しておいて、ボタンで表示させることができます。</span>',
	    'section' => 'global_section',
	    'type' => 'radio',
	    'choices' => array(
...
function opencage_toppage_theme_customizer( $wp_customize ) {
	...
	$wp_customize->add_control( 'opencage_toppage_header', array(
		...
		'description' => '<span style="font-size:10px;">設定に関わらず<b>トップページヘッダーを非表示</b>にします。</span>',
	...
	$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'opencage_toppage_headerimg', array(
		...
		'description' => '<span style="font-size:10px;"><span style="color:red;">画像を設定するとトップページに表示されます。</span>画像の上にテキストやボタンを配置したい場合は、下の「ヘッダーテキスト」の設定をしてください。</span>',
		'section'    => 'opencage_toppage_section',
	...
	$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'opencage_toppage_headerimgsp', array(
		...
		'description' => '<span style="font-size:10px;">スマートフォン用のヘッダー背景画像を設定できます。※設定しなかった場合は上の「ヘッダーイメージ」が表示されます。</span>',
		'section'    => 'opencage_toppage_section',
	...
	$wp_customize->add_control( 'opencage_toppage_headertext_l', array(
	    'label' =>'ヘッダーテキスト（大）',
		'description' => '<span style="font-size:10px;"><span style="color:red;">入力すると画像の上にテキストが表示されます。</span>大きな文字が表示されるので短いインパクトの有るキャッチコピー等を入力してください。</span>',
	    'section' => 'opencage_toppage_section',
	...
	$wp_customize->add_control( 'opencage_toppage_headertext_s', array(
	    'label' =>'ヘッダーテキスト（小）',
		'description' => '<span style="font-size:10px;"><span style="color:red;">入力すると画像の上にテキストが表示されます。</span>小さな文字で補足テキストが入ります。</span>',
	    'section' => 'opencage_toppage_section',
	    'type' => 'textarea',
	...
	$wp_customize->add_control( 'opencage_toppage_headerlink', array(
	    'label' =>'ヘッダーイメージURL',
		'description' => '<span style="font-size:10px;">リンクさせたいページやサイトのURLを入力すると、リンクが設置されます。</span>',
	    'section' => 'opencage_toppage_section',
...
function opencage_postpage_customizer($wp_customize) {
	...
	$wp_customize->add_control( 'post_options_postdesign', array(
	    ...
	    'label' =>'記事・固定ページレイアウト',
		'description' => '<span style="font-size:10px;">記事ページと固定ページのデザインを変更できます。※「ワンカラム」「バイラル風」を選択するとウィジェットの「PC:サイドバー」ウィジェットは表示されなくなります。</span>',
	...
	$wp_customize->add_control( 'post_options_ttl', array(
	    ...
	    'label' =>'見出しデザイン',
		'description' => '<span style="font-size:10px;">見出しのデザインを変更することが可能です。</span>',
	    'section' => 'postpage_section',
	    'type' => 'radio',
	    'choices' => array(
	...
	$wp_customize->add_control( 'sns_options_page', array(
		...
		'description' => '<span style="font-size:10px;">固定ページにも独自のSNSボタンを表示させたい場合にチェック。※固定フロントページを利用している場合にはトップページでは表示されません。</span>',
	    'section' => 'postpage_section',
	    'type' => 'checkbox',
	));
	...
	$wp_customize->add_control( 'sns_options_hide', array(
	    ...
	    'label' =>'SNSボタンを表示しない',
		'description' => '<span style="font-size:10px;">SNSボタンをプラグインなどで実装する場合に、チェックをいれればテーマ独自のSNSボタンが非表示になります。</span>',
	    'section' => 'postpage_section',
	    'type' => 'checkbox',
	));
	...
	$wp_customize->add_control( 'post_options_eyecatch', array(
		...
		'description' => '<span style="font-size:10px;">記事ページにてアイキャッチ画像が自動表示されなくなります。</span>',
	    'section' => 'postpage_section',
	    'type' => 'checkbox',
	));
	...
	$wp_customize->add_control( 'post_options_date', array(
	    ...
	    'label' =>'投稿日・更新日表示設定',
		'description' => '<span style="font-size:10px;">投稿日・更新日を非表示にすることができます。</span>',
	    'section' => 'postpage_section',
	    'type' => 'radio',
	    'choices' => array(
            'undo_off' => '投稿日のみ表示する',
	...
	$wp_customize->add_control( 'post_options_authordisplay', array(
	    ...
	    'label' =>'投稿者名の表示設定',
		'description' => '<span style="font-size:10px;">記事ページ（記事タイトル下）、記事一覧ページにて投稿者名を表示します。</span>',
	    'section' => 'postpage_section',
	    'type' => 'radio',
	    'choices' => array(
	...
	$wp_customize->add_control( 'twbox_options_url', array(
		...
		'description' => '<span style="font-size:10px;">リンクさせたいTwitterのURLを入力</span>',
	    'section' => 'postpage_section',
	));
	...
	$wp_customize->add_control( 'feedlybox_options', array(
		...
		'description' => '<span style="font-size:10px;">Feedlyボタンを表示させたい場合はチェックを入れてください</span>',
	    'section' => 'postpage_section',
	    'type' => 'radio',
	    'choices' => array(
            'feedly_on' => '表示する',
